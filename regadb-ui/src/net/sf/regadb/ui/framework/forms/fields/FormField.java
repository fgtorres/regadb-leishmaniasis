...
public abstract class FormField extends WContainerWidget implements IFormField, IFormTextField
{
	...
	public abstract WFormWidget getFormWidget();
    public abstract String getFormText();
    public abstract void setFormText(String text);
	...
	public static IFormField getTextField(ValueTypes type, InteractionState state, IForm form)
    {
        switch(type)
        {
        case STRING:
        ...
        case IMAGE:
        	ImageField image = new ImageField(state, form);
        	image.setMaximumSize(new WLength(400), new WLength(400));
        	Patient p = RegaDBMain.getApp().getTree().getTreeContent().patientTreeNode.getSelectedItem();
        	image.setFileName(Utils.generateFilename(p.getPatientId()));
        	return image;
        }
