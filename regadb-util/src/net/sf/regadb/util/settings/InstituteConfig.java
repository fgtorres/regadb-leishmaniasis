...
public class InstituteConfig extends ConfigParser {
	...
	private static final String CONTACT_FORM_NAME = "form.multipleTestResults.contact";
	
	private Filter organismFilter = null;
	private File logDir;
	private File queryResultDir;
	private File filesStoreDir;
	...
	private List<ContactFormConfig> multipleTestResultForms = new ArrayList<ContactFormConfig>();
	...
	public void parseXml(RegaDBSettings settings, Element e) {
		...
		ee = e.getChild("files-store-dir");
		if(ee != null)
			filesStoreDir = new File(ee.getTextTrim());
		...
		ee = e.getChild("multiple-test-result-forms");
				if(ee != null){
					for(Object oo : ee.getChildren()){
						Element cfc = (Element)oo;
						ContactFormConfig config = new ContactFormConfig(cfc.getAttributeValue("name"));
						config.parseXml(settings, cfc);
						multipleTestResultForms.add(config);
					}
				}
	...
	public void setDefaults() {
		...
		setFilesStoreDir(new File("/etc/rega_institute/regadb/files-store"));
		...
		addFormConfig(new ContactFormConfig(CONTACT_FORM_NAME));
	...
	public ContactFormConfig getContactFormConfig(){
		return (ContactFormConfig) forms.get(CONTACT_FORM_NAME);
	}	
	
	public List<ContactFormConfig> getMultipleTestResultForms() {
		return multipleTestResultForms;
	}
	...
	public void setFilesStoreDir(File filesStoreDir) {
		this.filesStoreDir = filesStoreDir;
	}

	public File getFilesStoreDir() {
		return filesStoreDir;
	}
