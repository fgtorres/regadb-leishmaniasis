...
public abstract class TestListWidget {
	...
	private List<IFormField> testFormFields = new ArrayList<IFormField>();
	...
	public void fillData(Set<TestResult> testResults) {
		...
		for(int i = 0; i < testItems.size(); i++) {
			...
			IFormField f = testFormFields.get(i);
			if (theTr != null) {
				if (f instanceof ComboBox) {
					((ComboBox) f).selectItem(theTr.getTestNominalValue().getValue());
				} else {
					if (theTr.getValue() != null)
						 ((IFormTextField)f).setText(theTr.getValue());
					else 
						 ((IFormTextField)f).setText(new String(theTr.getData()));
				}
			}
		}
	}
