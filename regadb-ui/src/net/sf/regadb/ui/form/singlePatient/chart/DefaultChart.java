...
import net.sf.regadb.util.settings.PatientChartConfig.Item;
...
public class DefaultChart {
	public static Chart createDefaultChart(WContainerWidget parent, Patient p, String y1, String y2, Date min, Date max, int width, int height) {
		...
		if(y1 != null && y2 != null){
			...
			cd4.name = y1;
			cd4.testTypes.add(new PatientChartConfig.TestType(y1));
			if (pcc.getY1().items.size()>0){
				pcc.getY1().items.remove(0);
			}
			
			pcc.getY1().items.add(cd4);
			
			Item vl = new Item();
			vl.kind = Item.Kind.CellCount;
			vl.name = y2;
			vl.testTypes.add(new PatientChartConfig.TestType(y2));
			
			if (pcc.getY2().items.size()>0){
				pcc.getY2().items.remove(0);
			}
			
			pcc.getY2().items.add(vl);
		}
		
		Chart chart = new Chart(parent, pcc.getY1().items.get(0).name, pcc.getY2().items.get(0).name, min, max);
		...
		int chartPaddingLeft = 200;
		int chartPaddingRight = 200;
		int chartPaddingBottom = chart.calculateAddedHeight() + 30;
